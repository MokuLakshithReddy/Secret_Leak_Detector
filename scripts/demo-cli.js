...
// Load compiled test-suite which exports scanContent, calculateShannonEntropy, redactSecret
const { scanContent, calculateShannonEntropy, redactSecret, buildScanResult } = require('../dist/test-suite');
...
async function runDemo() {
  ...
  console.log(`⚡ Scan completed in ${duration}ms! Found ${findings.length} credential(s).\n`);
  ...
  if (result.blocked) {
    console.log(`
🚨❌ COMMIT BLOCKED: Critical secrets detected in commit payload!
------------------------------------------------------------
Why:
  Hardcoded secrets detected before code leaves your machine.
